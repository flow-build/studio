...
export const ProcessStateDialog = ({ generateDiagram }) => {
  ...
  const [isSearchProcessIdDialogActive, searchProcessIdDialogData] =
    useSelector(({ bpmn }) => [
  ...
  const handleOnSearchHistory = () => {
    ...
    navigate(
      `/dashboard/workflows/${workflowId}/processes/${processId}/history`
    );
  };
  ...
  return (
    <Dialog onClose={handleOnClose} open={isSearchProcessIdDialogActive}>
      ...
      <DialogContent>
        ...
        <DialogContentText>
          Node ID: {searchProcessIdDialogData?.state?.node_id}
        </DialogContentText>
        <DialogContentText>
        ...
        <Stack direction="row" spacing={1}>
          <Chip
            label={searchProcessIdDialogData?.current_status}
            sx={{
              background:
                statusColors[`${searchProcessIdDialogData?.current_status}`],
            }}
            variant="outlined"
          />
        </Stack>
      ...
      <DialogActions sx={{ justifyContent: "space-between" }}>
        <Button onClick={handleOnDrawDiagram} variant="outlined">
          Ver no Diagrama
        </Button>
